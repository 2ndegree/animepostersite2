...
refreshlist(){
	...
	let html =`
	    <div class="">									 
	      <input oninput="component.searchanime(this.value)" id="textsearchanime" type="text" class="form-control" placeholder="Search">
	    </div>
	    <br>
	    <br>
	 `;
...
updateanime(key){
	...
	searchanime(key){
	    ...
	    let listanime = document.getElementById("animes");
		...
		for(let i=0;i<m.length;i++){	
			html += `
				<tr>
					<td>${m[i].title}</td>
					<td>${m[i].year}</td>
					<td><button class="btn btn-primary"  onclick="component.animeDetails(${i})">Show Details</button></td>
				</tr>
			`;
		}
		this.reRender(html,document.getElementById('list'));
	}
